$(document).ready(function() {
  tableau.extensions.initializeAsync({ configure: configure }).then(function() {
    ...
    const savedSheetName = tableau.extensions.settings.get("sheet"); // TODO: source data should be split between nodes and edges (2 source sheets required)
    const savedSourceNodeField = tableau.extensions.settings.get("source_node");
    const savedSourceTypeField = tableau.extensions.settings.get("source_type");
    const savedTargetNodeField = tableau.extensions.settings.get("target_node");
    const savedTargetTypeField = tableau.extensions.settings.get("target_type");
    if (
      savedSheetName &&
      ...
      const inputWs = getSelectedSheet(savedSheetName);
      ...
      // settings not set, manual placeholders
