...
function renderExtension() {
  ...
  var savedSheetName = tableau.extensions.settings.get("sheet"); // TODO: source data should be split between nodes and edges (2 source sheets required)
  var savedSourceNodeField = tableau.extensions.settings.get("source_node");
  var savedSourceTypeField = tableau.extensions.settings.get("source_type");
  var savedTargetNodeField = tableau.extensions.settings.get("target_node");
  var savedTargetTypeField = tableau.extensions.settings.get("target_type");
  var sigmaInstance;
  var tooltips;
  var lasso;
  var select;
  ...
  if (
    ...
    // const detailWS = getSelectedSheet(savedDetailsSheetName);
    ...
    unregisterEventHandlerFunction = inputWs.addEventListener(
...
function getSelectedSheet(worksheetName) {
  return tableau.extensions.dashboardContent.dashboard.worksheets.find(function(
    sheet
  ) {
  ...
  tableau.extensions.ui
    ...
    .then(function(closePayLoad) {
      if (!tableau.extensions.settings.get("sheet")) {
        ...
        //renderExtension(); // this is catched by the settings changed event
      }
    })
